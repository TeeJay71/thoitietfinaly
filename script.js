...
function createHTML(cityName, countryName, windspeed, tempValue){
	var bgClass;
	if (tempValue >= 90){
		bgClass = 'redBg';
	}
	...
	var htmlString =	'<div class="setBorder ' + bgClass + '">' +
											'<div class="weatherCity">Thành phố ' + cityName + '</div>' +
											'<div class="weatherCity">Nước ' + countryName + '</div>' +
											'<div class="weatherCity">Tốc độ gió ' + windspeed + ' km/h</div>' +
											'<div class="weatherData">' + tempValue + '°C</div>' +
										'</div>';
...
var searchWeather = function(city){
	...
	$.ajax({
		...
		success: function(data){
			...
			var theWind = data.wind.speed;
			var theTemp = Math.round((data.main.temp-32)*5/9);

			//Call a function that will create an HTML string & add it to the page
			createHTML(theCity, theCountry, theWind, theTemp);
		}
	});
};
